...
Lors d'un appel à \ocamlf{remove_products} pour barrer tous les multiples de $n$,
...
au moment où la boucle de \ocamlf{remove_products} arrive sur lui, peut très bien
être déjà marqué. Si c'est le cas, alors il aura nécessairement été marqué par
une précédente itération de la boucle. % à voir si le terme est correct ...
...
seront pas réexaminés par la fonction \ocamlf{remove_products} et qui,
...
Afin de diviser l'espace utilisé par deux, le tableau \ocamlf{arr} ne représente que les nombres
...
% ceci a notamment permis de simplifier quelques points de la preuve
% à la fin, lors de l'extraction des nombres premiers.

L'implémentation de l'algorithme que nous allons prouver se trouve dans la
figure~\ref{fig:codeOCaml}. Dans celle-ci, on manipule à plusieurs reprises
un type \ocamlf{t}. Ce type contient pour premier champ le tableau \ocamlf{arr}
décrit précédemment. Le champ \ocamlf{max} correspond à la limite donnée en
entrée au crible et le champ \ocamlf{max_arr} correspond au plus grand indice
du tableau \ocamlf{arr}, donc $\text{\ocamlf{max_arr}} \eqdef \frac{\text{\ocamlf{max}} - 1}{2}$.
...
La première postcondition représente le fait que les nombres premiers
du tableau renvoyé en résultat sont écrits dans l'ordre croissant.
La postcondition suivante montre que les nombres premiers renvoyés dans
le tableau sont tous dans l'ensemble $\{2,...,\text{\ocamlf{max}}\}$.
Enfin, les deux dernières postconditions présentent énoncent que tous les
...
l'implémentation. Ce type se trouve dans le module \whyf{EulerSieve}
...
\end{why3}
Les types utilisés pour décrire la liste chaînée et le marquage sont ici des
séquences. Il s'agit de tableaux immuables fournis par la bibliothèque standard
de Why3. Pour pouvoir les manipuler il est donc nécessaire de rendre les champs
correspondants mutables.
...
Ces champs fantômes sont ici très utiles puisqu'ils permettent de faciliter
la preuve et seront supprimés lors de l'extraction du programme,
une fois démontré, vers du code OCaml.
Des fonctions permettant de modifier ces champs ont été écrites.
Elles sont définies de manière abstraites dans le module \whyf{EulerSieve}
et seront implémentées dans le module \whyf{EulerSieveImpl}.
