...
\clearpage
\begin{figure}[tp]
\lstinputlisting[language=ocaml, firstline=0, lastline=32]{decoupage_code.ml}
...
code précédente sont donc les suivantes :
\lstinputlisting[language=ocaml, firstline=36, lastline=39]{decoupage_code.ml}
...
consiste à remplacer le contenu de la case~$i$ par son opposé. On a donc le code
suivant pour les fonction \ocamlf{get_mark}, \ocamlf{set_mark} et
\ocamlf{create} :
\lstinputlisting[language=ocaml, firstline=41, lastline=43]{decoupage_code.ml}

\clearpage
\lstinputlisting[language=ocaml, firstline=47, lastline=53]{decoupage_code.ml}
...
Enfin, en fin de l'algorithme, on souhaite renvoyer un tableau ne contenant que
...
premiers. Le code suivant, qui correspond à la fin du code de la
figure~\ref{fig:codeOCaml}, représente cette extraction des nombres premiers.
...
reprises un type \ocamlf{t}. Ce type contient pour premier champ le tableau
